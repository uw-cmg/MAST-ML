...
class TestSelectors(unittest.TestCase):
    ...
    def test_featureSelectorWithScore(self):
        target = 'E_regression.1'
        extra_columns = ['Material compositions 1', 'Material compositions 2', 'Hop activation barrier', 'E_regression']
        d = LocalDatasets(file_path='mastml/tests/unit_tests/figshare_7418492/All_Model_Data.xlsx',
                          target=target,
        ...
        y = data_dict['y']
        model = RandomForestRegressor()
        selector = EnsembleModelFeatureSelector(model=model, n_features_to_select=100, n_dummy_variable = 10)
        Xselect = selector.evaluate(X=X, y=y, savepath=os.getcwd())
