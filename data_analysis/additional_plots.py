...
def cross_validation_full_fit_plot(dpath):
    ...
    kfold_data = np.genfromtxt(os.path.join(dpath,"KFoldCV/KFoldCV_data.csv"), 
                    dtype='float', delimiter=',', names=True) 
    ...
    gmax = max(xmax, ymax)
    plt.axis('equal')
    plt.plot(np.arange(gmin+1,gmax-1,10),np.arange(gmin+1,gmax-1,10),':',color="gray")
...
def cross_validation_full_fit_plot_sameranges(dpath):
    matplotlib.rcParams.update({'font.size': 18})
    fullfit_data =np.genfromtxt(os.path.join(dpath,"FullFit/FullFit_data.csv"), 
                    dtype='float', delimiter=',', names=True) 
    kfold_data = np.genfromtxt(os.path.join(dpath,"KFoldCV/KFoldCV_data.csv"), 
    ...
    plt.ylabel("GKRR predicted $\Delta\sigma_{y}$ (MPa)")
    plt.tight_layout()
    plt.savefig(os.path.join(dpath,"cross_validation_full_fit_sameranges.png"))
    plt.close()
    return

def cross_validation_full_fit_plot_sameranges_vert(dpath):
    matplotlib.rcParams.update({'font.size': 18})
    fullfit_data =np.genfromtxt(os.path.join(dpath,"FullFit/FullFit_data.csv"), 
                    dtype='float', delimiter=',', names=True) 
    kfold_data = np.genfromtxt(os.path.join(dpath,"KFoldCV/KFoldCV_data.csv"), 
    ...
    gmax = max(xmax, ymax)
    plt.axis([gmin, gmax, gmin, gmax])
    plt.xticks(np.arange(gmin, gmax+51.0, 200))
    plt.yticks(np.arange(gmin, gmax+51.0, 200))
    plt.plot(np.arange(gmin+1,gmax-1,10),np.arange(gmin+1,gmax-1,10),':',color="gray")
    ...
    plt.ylabel("GKRR predicted $\Delta\sigma_{y}$ (MPa)")
    plt.tight_layout()
    plt.savefig(os.path.join(dpath,"cross_validation_full_fit_sameranges_vert.png"))
    plt.close()
    return

if __name__=="__main__":
    dpath = "../../../data/DBTT_mongo/data_exports_dbtt_41_copycsv_singlep_20170224/1expt"
    dpathabs = os.path.abspath(dpath)
    cross_validation_full_fit_plot_sameranges_vert(dpathabs)
